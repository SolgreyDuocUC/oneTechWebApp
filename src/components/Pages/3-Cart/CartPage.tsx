...
export const CartPage = ({ onNavigate }: CartPageProps) => {
  ...
  const handleApplyCode = () => {
    const code = codigoReferido.toUpperCase();
    if (code === 'GAMER2024') {
      setDescuento(subtotal * 0.1);
      toast.success('¡Código aplicado! 10% de descuento');
    } else if (code === 'ADMIN2024') {
      setDescuento(subtotal * 0.15);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-8">Agrega productos a tu carrito para continuar comprando</p>
          <Button
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[var(--neon-green)] text-xl">{formatPrice(product.price)}</div>
                  </div>
                  <div className="flex flex-col items-end justify-between">
                    <div className="flex items-center gap-2 bg-[#1a1a1a] rounded-lg">
                      ...
                      <span className="text-white min-w-[2rem] text-center">{item.cantidad}</span>
                      <button
                        onClick={() => updateQuantity(item.productId, item.cantidad + 1)}
                        className="p-2 text-gray-400 hover:text-[var(--neon-green)] transition-colors"
                    ...
                    <div className="text-right">
                      <div className="text-white mb-2">
                        Subtotal: {formatPrice(product.price * item.cantidad)}
                      </div>
                      <button
                        onClick={() => {
