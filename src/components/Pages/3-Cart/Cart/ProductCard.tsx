      ...
      <div className="p-4">
        ...
        {avgRating > 0 && (
          <div className="flex items-center gap-1 mb-3">
            ...
            <span className="text-[var(--neon-green)] text-sm">{avgRating.toFixed(1)}</span>
            <span className="text-gray-500 text-xs">({productReviews.length})</span>
          </div>
        )}
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-2xl text-[var(--neon-green)]">{formatPrice(product.price)}</div>
            <div className="text-xs text-gray-500">Stock: {product.stock} unidades</div>
          </div>
        </div>
